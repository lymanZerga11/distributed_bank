...
int udp_server::recvfrom(char *msg, size_t max_size, struct sockaddr_in &si_other, socklen_t &slen)
{
  // struct sockaddr_in si_other;
  // int slen = sizeof(si_other);
  int packet_length = 0;
  if (packet_length = (recvfrom(server.get_socket(), msg, max_size, 0, (struct sockaddr *) &si_other, (socklen_t *) &slen)) != -1)
  {
...
int udp_server::sendto(const char *msg, size_t size)
{
    return sendto(f_socket, msg, size, 0, f_addrinfo->ai_addr, f_addrinfo->ai_addrlen);
}
