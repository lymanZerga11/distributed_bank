...
void Server::process_messages () {
  while (true) {
    ...
    if (udp_server.recvfrom(request_packet, max_size, (struct sockaddr *) &si_other, (socklen_t *) &slen) == packet_size) {
      ...
      udp_server.sendto(response_message.serialize(), max_size, to_address);
    }
